...
type Props = {
	onSelectMember?: (member: OrderMemberSearchResponseDto) => void;
...
const AddMemberModal = ({ onSelectMember, handleCloseModal }: Props) => {
	...
	// 處理選擇會員
	const handleSelectMember = (member: OrderMemberSearchResponseDto) => {
		if (onSelectMember) {
								...
								>
									...
									<Stack direction='row' alignItems='center' spacing={1.5} sx={{ width: 256 }}>
										<Box sx={{ position: 'relative', width: 40, height: 40 }}>
											<Avatar
												src={member.member.avatar || undefined}
												alt='avatar'
													...
													label={
														<Box component='span' sx={{ display: 'inline-flex', alignItems: 'baseline', gap: 0.5 }}>
															<Typography component='span' variant='caption' sx={{ lineHeight: 1.25, fontWeight: 400 }}>
																單板LV.{member.member.skis} 雙板LV.{member.member.snowboard}
															</Typography>
															<Typography
																component='span'
																variant='caption'
