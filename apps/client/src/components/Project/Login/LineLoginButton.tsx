...
const LineLoginButton = ({ theme = 'green' }: Props) => {
	...
	return (
		<>
			{theme === 'green' ? (
				<button
					className='flex overflow-hidden gap-2 justify-center items-center px-6 py-3 w-full text-base font-bold text-white bg-[#1AC460] hover:bg-[#159C4D] rounded-lg max-xs:px-5 transition-all duration-200'
				...
				<button
					className='flex overflow-hidden gap-2 justify-center items-center px-5 py-2 w-[127px] text-sm text-white bg-zinc-800 border-1 border-zinc-800 border-solid hover:bg-zinc-700 rounded-lg max-xs:px-5 transition-all duration-200 h-10'
					onClick={handleLineLogin}
				>
					<span>透過LINE登入</span>
				</button>
